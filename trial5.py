...
prep_step.add(Flatten())
prep_step.add(Dense(seed_dim))
prep_step.add(Activation('tanh'))

mnemonic_input = prep_step(img)
...
tmp = os.path.join(save_dir, 'latent1.h5')
latent_model = load_model(tmp)
latent_model.trainable = False

# MNEMONIC DEVICE
mnist_img = generator_model(mnemonic_input)
